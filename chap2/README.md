...
　②uts名前空間の作成(システムコールのunshareコマンドを使います)  
> unshare -u  

　③再び名前空間一覧を表示すると、utsのinode番号が変わっていることが分かる  
> ls -l /proc/$$/ns
...
　②chrootコマンドにて、/home/userYYディレクトリのみ参照できるようにします　　
> MYROOT=/home/userYY  
...
（１）では、CPU使用率を制限しましたが、こちらは、複数のコンテナ間で利用するCPUの共有割合を指定するやり方になります  
　cpu.sharesを使います  
...
　①まず、実験用ディレクトリを作成
> mkdir -p /home/userYY/overlay  
> cd /home/userYY/overlay  
...
　abc.txtおよびxyz.txtが見れる  
　両者はそれぞれ、lower1、lower2に実体があるが、mergedディレクトリに並んであるように見えます  
...
　mergedディレクトリ上には、abc.txt、xyz.txtと並んで、hoge.txtができており  
...
　mergedディレクトリ上には、abc.txtが更新され、  
　また、upperディレクトリ上にもabc.txtが出現して、変更されたことが分かります  
...
　mergedディレクトリ上には、xyz.txtが削除されますが、  
　upperディレクトリ上では、xyz.txtが残った状態となります  
...
　こちらのコマンドでイメージ内容を確認できます  
　その中に、lower,upper,mergedの記載があります  

　これらは、/var/lib/docker/overlay2/  
　以下に展開されて、普通のディレクトリとしてみることが可能です  
